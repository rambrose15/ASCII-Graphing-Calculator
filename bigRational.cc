...
class BigNum {
    vector<int> digits; // Largest digit at back, smallest digit at front
    size_t len;
    bool negative;
    BigNum(vector<int> digits, bool negative): 
    ...
    vector<int> addUnsigned(const BigNum& other) const {
        using ll = long long;
    ...
    vector<int> subUnsigned(const BigNum& other) const {
        using ll = long long;
    ...
    BigNum(string digitString = "0") { // Assumes the string has only numerics or a negative in the front
    ...
    void gcdReduce(BigNum& other){ // Reduces both itself and the argument by their GCD
    ...
    private: BigNum findGCD(const BigNum& a, const BigNum& b){
...
class BigRational{
    ...
    BigRational(const string& decimalString){ // Ctor to get BigRational from a string eg. 132.10210
